...
fetch_nhdv2_attributes_from_sb <- function(vars_item, save_dir, comids, 
  ...
  # download_sb_file() is a function defined in 1_fetch/src/download_file.R
  ...
  col_names <- vars_item %>%
    ...
    lapply(., function(x){
      if(x$attribute_name == "sinuosity"){
        names <- c("COMID","sinuosity")
      } else {
        names <- c("COMID",
  ...
  data_out <- lapply(out_file, unzip_and_clip_sb_data,
    ...
    Reduce(full_join,.) %>% 
    # suppress messages from Reduce about joining by "COMID" column
    suppressMessages() 
  
  # 5) Check that all requested attributes are represented in data_out
  check <- lapply(unique(vars_item$attribute_name), function(x){
    check_name <- grepl(x, names(data_out))
    
    if(!any(check_name)){
      warning(sprintf(paste0("We didn't find attribute %s in the column names ",
                             "of our compiled dataset. Check that the attribute ",
                             "name in the data release matches what we expect ",
                             "(e.g. 'CAT_x', 'ACC_x', or 'TOT_x')."),
                      x))
      }
    })
...
unzip_and_clip_sb_data <- function(out_file, col_names, comids, save_dir, 
  ...
  if(!grepl(".zip", out_file)){
    warning(sprintf(paste0("We downloaded %s from ScienceBase but it's not a zip ",
    "file and so we're skipping over it when we read in the NHDPlusv2 attributes."), 
    out_file))
    
    # if out_file is not a zip file, skip the steps below and remove
    # the file if delete_local_copies is TRUE
    dat <- NULL
    if(delete_local_copies){
    ...
    # Remove files if delete_local_copies is set to TRUE
    if(delete_local_copies){
      file.remove(out_file)
